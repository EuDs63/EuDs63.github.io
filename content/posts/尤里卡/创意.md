...
## 贴吧存档 
- 时间：2024年7月8日
- 想法：贴吧上还是有些帖子值得保存的。把它们保存下来会挺有意义的。虽然我的[EuDs63/Tieba_Memory](https://github.com/EuDs63/Tieba_Memory )已经能做到了，但我还是嫌麻烦。能不能做成个服务。我发给机器人链接，或者是做成个网站，输入链接就生成pdf呢 
- 可行性分析：
  - 前者不难做，后者如果能做成webassembly的纯前端的话，那么部署成本应该不高。
  - 查了下python to wasm 的资料，发现[pyodide/pyodide](https://github.com/pyodide/pyodide )似乎能用
...
## 自动rss订阅链接生成 
- 时间: 2024年9月2日
- 想法：逛博客的时候还是能发现不少博客没有rss链接的，虽然[DIYgod/RSSHub-Radar: 🧡 Browser extension that simplifies finding and subscribing RSS and RSSHub](https://github.com/DIYgod/RSSHub-Radar )能够嗅探，但奈何有些博客就是没有。如果能写一个一键生成rss订阅链接的就好了。
- 可行性分析：
...
## 爬虫练习
- 时间: 2024年11月14日
- 想法: [教育部臺灣台語動畫|網站介紹](https://twbangga.moe.edu.tw/classification )提供了对部分日本动画的台语配音，质量很不错。出于收藏的目的，我想下载下来。
- 可行性分析:
  - 出于版权考虑，它仅限于台湾地区能看。是靠检查ip，检查后将地区存在localstorage，key为`isCountry`，值就是Hk或者Taiwan，看着很搞笑。但修改它没用，因为再次点击的时候还是会检查。最简单的方法还是靠换个当地的代理节点。
  - 视频爬取，[Java 爬虫练习-bilibili视频下载 索引](https://nicelee.top/blog/2019/03/02/java-spider-bilibili-down-index/ )写得很好，把下载b站视频的过程循序渐进的给讲明白了。我一个多月前看完就想自己试试，这下有机会了。
- 实现情况: 已实现,见[EuDs63/TwbanggaDownload](https://github.com/EuDs63/TwbanggaDownload )

---

## B站评论分析
- 时间: 2024年11月15日
- 想法: 我很不爽b站的一点是它不提供历史评论，我翻不到自己过去在哪，回复了什么内容。我希望自己能获取到自己的历史评论。并进行一定的分析。
- 可行性分析:
  - 发现[查评论](https://www.aicu.cc/reply )可以查。我可以通过它获取到历史评论信息。
  - 我可以参照[EuDs63/Tieba_Memory: 导出百度贴吧个人回复记录，并进行简单的数据分析](https://github.com/EuDs63/Tieba_Memory )写个bili_memory，处理下数据就能用。
  - 我还可以写个bot插件,用来查成分。
- 实现情况: 已实现,见[EuDs63/BiliBili_Memory: 导出B站个人评论记录，并进行简单的数据分析](https://github.com/EuDs63/BiliBili_Memory )
...
- 可行性分析:
  ...
  - 可以参考[Buff购买记录总价计算](https://greasyfork.org/zh-CN/scripts/463576-buff%E8%B4%AD%E4%B9%B0%E8%AE%B0%E5%BD%95%E6%80%BB%E4%BB%B7%E8%AE%A1%E7%AE%97 )

---

## AI取名
- 时间: 2025年2月3日
- 想法: 某日聊天玩梗，说看到xxx的时候，连孩子叫啥都想好了。被问到叫啥，还真没想好。取个名是真不容易，于是想到了AI。虽不能决策，参考下总是不错。那就做个AI取名的网站吧。至少得有这几个功能吧：
  1. 姓的输入
  2. 寓意，期望的输入
  3. 输出时显示名字的寓意，以及名字可能的重名情况
- 可行性分析:
  - 得益于llm的强大,网站实现起来不难,但要做好不容易。因为我觉得这网站最大的一个问题是，网站的意义？用户为啥不直接跟llm聊，而是用你这个网站。目前我没想好，更好的用户体验？不用自己输复杂的prompt？
  - 姓名的重名情况，我之前有用过类似的服务，但不知道有没有合法的api让我使用。
  - 如何尽可能地不被prompt 注入攻击，也是一个问题。 姓的话可以做规则过滤，毕竟姓是有限的，但寓意呢，因为需要允许用户输入他的期望。

---

## AI群友
- 时间: 2025年2月4日
- 想法: 小群里的一位群友因故无法发言，大家都挺想ta的。在得到其授权的前提下，想着用ai模仿一个出来，以表怀念。
- 可行性分析:
  - 给llm喂聊天记录等资料，来训练出一个说法风格类似，甚至逻辑有一定相似的bot，并不算什么新鲜事。至少挺早之前[李萌萌 (@ljsabc) / X](https://x.com/ljsabc )就做过。
  - 难点在于语料。如何导出并解析qq聊天记录，并进行清洗，训练？